...
public class Dijkstra {
  ...
  public void doDijkstra(String s) {
      ...
      for(Vertex v: vertexNames.values()){
          v.distance = Double.NaN;
          v.prev = null;
          v.known = false;
      }
      
      Vertex origin = vertexNames.get(s);
      origin.distance = 0;
      
      boolean allKnown = false;
      
     
      while(allKnown == false){
          Vertex smallestVertex = null;
          for(Vertex v: vertexNames.values()){
 
              if(v.known == false && !Double.isNaN(v.distance) && smallestVertex == null){
                  smallestVertex = v;
              }else if(v.known == false && !Double.isNaN(v.distance) && v.distance < smallestVertex.distance){
                      smallestVertex = v;
              }else{
                  
              }
          }
          for(Edge e: smallestVertex.adjacentEdges){
              Vertex adjVert = e.target;
              Vertex prevVert = e.source;
              if(adjVert.known == false){
                  double tempDist = e.distance + prevVert.distance;
                  if(Double.isNaN(adjVert.distance) || tempDist < adjVert.distance){
                      adjVert.distance = tempDist;
                      adjVert.prev = prevVert;
                  }
              }
  ...
  public List<Edge> getDijkstraPath(String s, String t) {
    ...
    Vertex currentVert = vertexNames.get(t);
    Vertex startVert = vertexNames.get(t);
    while(currentVert.prev != null){
        for(Edge e: currentVert.prev.adjacentEdges){
            if(e.target.equals(currentVert))
                route.addFirst(e);
        }
